...
cat(sprintf("library(%s)", packages), file = path(d_src, "packages.R"), sep = "\n")
...
devtools::install_github("JouniVatanen/stools"); library(stools)
...
#data("fi_postnumber_2016", package = "stools")
...
fi_readr <- readr::locale(date_names = "fi", decimal_mark = ",", 
                          grouping_mark = " ")
fi_vroom <- vroom::locale(date_names = "fi", decimal_mark = ",", 
                          grouping_mark = " ")
na_values <- c("", "NA", "-'")
...
rmarkdown::render("Rproj-template.Rmd", "html_document", output_dir = d_out)
```
...
df_db <- dbGetQuery(con, sql)
dbDisconnect(con_eetu)
...
df_numeric <- df %>%
  ...
  mutate_at(
    vars(1:3), 
    ~as.numeric(parse_factor(.x, statement_levels, 
...
df_desc_raw <- desc_stat(df_numeric, c(1:10), c("var1", "var2"))
```
...
df_desc <- df_desc_raw %>%
  ...
  mutate_at(
    ...
    ~case_when(
      ...
      .x == "0" ~ "neutraali",
      .x == "-100" ~ "kriittinen",
      .x == "1" ~ "täysin eri mieltä",
      .x == "2" ~ "jokseenkin eri mieltä",
      .x == "3" ~ "ei samaa eikä eri mieltä",
      .x == "4" ~ "jokseenkin samaa mieltä",
      .x == "5" ~ "täysin samaa mieltä",
      TRUE ~ .x))) %>%
...
df_to_txt(df_desc, path(d_out, "desc_table.txt"))
df_to_txt(df_feedback, path(d_out, "data_feedback.txt"))
df_to_txt(df, path(d_out, "data_cleaned_2018.txt"))
```
...
set.seed(09082019)
plot_wordcloud(df_feedback$feedback, path(d_out, "wordcloud.png"))
```
